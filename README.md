...
> 本项目衍生自 [xjetry/nft-forward](https://github.com/xjetry/nft-forward)，现作为独立项目 `nft` 由 **cheesydi-cloud** 维护。原始版权归上游作者所有，本仓库以 MIT 许可证发布（见 [LICENSE](./LICENSE)）。
...
```bash
sudo bash <(curl -fsSL https://raw.githubusercontent.com/cheesydi-cloud/nft/main/install.sh)
```

直接指定角色（管道模式）：

```bash
curl -fsSL https://raw.githubusercontent.com/cheesydi-cloud/nft/main/install.sh | sudo bash -s server
