...
1. 启动 daemon：
   ...
   sudo nft daemon
   ```
   预期输出：`nft daemon: listening on /var/run/nft.sock`
...
4. 在 panel 确认 `localhost` 节点已自动注册，其地址为 `unix:///var/run/nft.sock`。

5. 通过 panel UI 在 localhost 节点上创建一条 forward。

6. 在终端验证 panel 创建的 forward 出现在 daemon 的 `panel` segment 中：
   ```bash
   curl -s --unix-socket /var/run/nft.sock http://daemon/v1/ruleset
   # → {"owners":{"panel":[{...}]}}
   ```
...
2. 在第二个主机上启动 agent 模式的 daemon：
   ```bash
   # 写 panel token 文件
   echo "<panel 生成的 token>" | sudo tee /etc/nft/panel.token
...
- **Socket 权限** — unix socket 只有文件权限控制（生产部署只让 root + nft group 用户能连）
...
ls -l /etc/nft/panel.token    # 记录 inode 和 mtime
sudo bash install.sh update
ls -l /etc/nft/panel.token    # 应当与上面一致，inode 不变
...
# 1. 装 daemon + 加一条规则
sudo bash install.sh tui
sudo nft    # 添加规则 58443 → 10.20.1.20:8443，退出

# 2. 验证 DOCKER-USER 出现 owner-tagged 规则
sudo nft list chain ip filter DOCKER-USER | grep "nft managed"
#   ct state established,related counter ... accept comment "nft managed"
#   ip daddr 10.20.1.20 tcp dport 8443 counter accept comment "nft managed"

# 3. 真实客户端从外网测：应该能连通（之前的"DNAT 后 SYN 没出去"问题消失）

# 4. 停 daemon，验证清理
sudo systemctl stop nft-daemon.service
sudo nft list chain ip filter DOCKER-USER | grep "nft managed" && echo "FAIL: 残留" || echo "OK: 已清理"
```
