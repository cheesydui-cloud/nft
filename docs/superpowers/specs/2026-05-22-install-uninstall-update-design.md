...
1. **没有 update 路径**。重跑 `install.sh tui` 实际上会覆盖 `/usr/local/sbin/nft`（`install -m 0755`），但这是侧门——没有架构防护、没有备份、新二进制起不来时没有回滚。
...
- **不**把这些能力迁移到 `nft` 二进制子命令。bootstrap 永远需要外部脚本，让 Go 重写一遍 systemd unit 模板/角色识别/包管理代码是把代码量翻倍但能力不增加。
...
- **purge 不主动关 `net.ipv4.ip_forward`**。只删 `/etc/sysctl.d/99-nft.conf`（reboot 后回到内核默认），当前运行时的 ip_forward 保留给系统管理员决定，避免误伤其它依赖转发的服务。
...
| 信号 | 来源 |
|---|---|
| server unit 是否存在 | `systemctl list-unit-files --no-legend \| grep -q '^nft-server\.service '` |
| daemon 是否带 `--listen`（即 agent 角色） | `grep -q -- '--listen' "$SYSTEMD_DIR/nft-daemon.service"`（仅在 unit 文件存在时执行） |
...
**重要不变量**：自动清旧只动 unit/token，不动 `/var/lib/nft/state.json`。这意味着从 server/agent 降回 tui 时，旧的 `panel` 段规则保留在 daemon 内继续生效——用户若想清这些规则，必须显式跑 `install.sh uninstall <旧角色> --purge` 后再 install。spec 在 README 升级章节明确告知这一点。
...
**1. 前置探测**
- `/usr/local/sbin/nft` 不存在 → die（"未安装"）
- `nft-daemon.service` 不在 unit-files 列表 → die（同因）
- 用户传了 `--release` → die（"update 只拉 latest，要锁版本请用 install"）

**2. 架构防护**
- `uname -m` 非 `x86_64`/`amd64` → die
- 下载完成后跑 `file "$tmp/nft"`，输出含 `ELF 64-bit LSB executable, x86-64` 才继续；不匹配 → die，不进入替换
...
- SHA256SUMS 校验失败 → die
- 通过校验后跑 `"$tmp/nft" --help`（main.go 没有 `--help` flag，Go 的 flag 包默认行为是打印 usage 退 2，足够证明二进制能被执行——非 0 退出码不是失败信号，**只要 exec 不报 "exec format error" 就算通过**）

**4. 备份旧二进制**
- `cp -a /usr/local/sbin/nft /usr/local/sbin/nft.bak`
- 设 trap：`trap 'rollback' ERR INT TERM`，`rollback()` 函数定义见步骤 8

**5. 原子替换**
- `install -m 0755 "$tmp/nft" /usr/local/sbin/nft`
...
- 如果 `nft-server.service` 在 unit-files 列表里 → `systemctl restart nft-server.service`
...
rollback() {
  ...
  mv -f /usr/local/sbin/nft.bak /usr/local/sbin/nft
  systemctl restart nft-daemon.service
...
**9. 成功收尾**
- `rm -f /usr/local/sbin/nft.bak`
- `ok` 打印新二进制 sha256、size、提示用户检查 `journalctl -u nft-daemon.service`

**关键不变量**：
- update 不动 unit 文件内容 → 用户的 `--listen`、`--addr` 配置不会丢
- update 不动 `/etc/nft/daemon.token`
...
|---|---|---|
| `server` | disable+stop `nft-server.service`、rm unit、daemon-reload | (1) **先**清 daemon 的 panel 段：`curl --unix-socket /var/run/nft.sock -X POST -H 'Content-Type: application/json' http://daemon/v1/ruleset/panel -d '{"rules":[]}'`（best-effort，daemon 若不响应记 warning 不阻塞）<br>(2) `rm -f /var/lib/nft/panel.db panel.db-wal panel.db-shm` |
| `agent` | 把 daemon unit ExecStart 改回无 `--listen`、rm `/etc/nft/daemon.token`、daemon-reload + restart daemon | (1) **先**清 daemon 的 panel 段（同上 API，在 daemon-reload + restart 之前发，确保持久化到 state.json）<br>(2) `rm -rf /etc/nft/` |
| `daemon` | 前置 guard：要求先卸 server；disable+stop daemon unit、rm unit、rm 二进制、daemon-reload | (1) `rm -rf /var/lib/nft/`<br>(2) `rm -f /etc/sysctl.d/99-nft.conf`<br>(3) `nft delete table ip nft_forward 2>/dev/null \|\| true`（best-effort 清残留表）<br>(4) `getent group nft >/dev/null && groupdel nft \|\| true`<br>(5) stderr 提示：若有 tc HTB 限速残留请手动 `tc qdisc del dev <iface> root` |
...
- agent `--purge`：先 POST panel rules=[] → write_daemon_unit "" → rm token → daemon-reload + restart daemon → rm -rf /etc/nft/
...
5. **update 成功路径**
   - 容器内启 `python3 -m http.server 8000`，目录里放 `nft`（同一镜像的二进制）+ 现场生成的 `SHA256SUMS`
   - `NFTF_RELEASE_BASE_URL=http://127.0.0.1:8000 install.sh update`
   - 断言：退出码 0；daemon 仍 active；`/v1/health` 仍 ok；`/usr/local/sbin/nft.bak` 不存在

6. **update 失败回滚**
   - http server 这次返回故意坏的二进制（如 `printf 'broken' > nft`，同步重算 SHA256SUMS 让 sha 校验过——制造"sha 过但 exec 不了/health-check 不过"的失败模式）
   - `install.sh update`
   - 断言：退出码 1；当前 `nft` 的 sha256 与 update 之前一致（已回滚）；daemon 仍 active；`.bak` 不存在（rollback 成功后已 mv 回去）
...
8. **uninstall daemon --purge**（必须在 7 之后执行；server 已不存在）
   ...
   - 断言：`/var/lib/nft/`、`/etc/sysctl.d/99-nft.conf` 都不存在；`nft list table ip nft_forward` 返回 not-found；`getent group nft` 返回空
...
- agent 角色 update（含 `/etc/nft/daemon.token` 文件保留断言）
...
| `NFTF_RELEASE_BASE_URL` 被滥用注入恶意二进制 | 需 root；不文档化；同等条件下攻击者直接替换 `/usr/local/sbin/nft` 更直接，本变量并非新增攻击面 |
