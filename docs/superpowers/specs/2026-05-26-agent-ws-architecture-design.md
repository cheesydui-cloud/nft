...
`nft` 当前的 server / agent 通信是 **server 主动 push**：
...
| `cmd/nft/main.go` `runDaemon`（改） | 去掉 `--listen` / `--token-file`；新增 `--connect` / `--panel-token-file` |
...
VALUES ('self', 'unix:///var/run/nft.sock', '', 'self', 1, NOW())
...
--               self node 存 'unix:///var/run/nft.sock' 表意
...
# agent : write_daemon_unit " --connect wss://panel/v1/agents --panel-token-file /etc/nft/panel.token"
```
...
`https → wss`、`http → ws` 自动转换，append `/v1/agents` 路径。token 写入 `/etc/nft/panel.token` (mode 0600)。
...
- `systemctl restart nft-server.service` → agent 60s 内重连
