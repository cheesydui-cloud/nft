# nft 二进制拆分设计：nft-server / nft-agent
...
当前 `nft` 是单一二进制，含面板（server）、守护进程（daemon/agent）、TUI 三种角色，约 12.7MB。节点侧（agent/tui）安装在大量边缘机上，体积越小越好。
...
- **取 agent 字节**：新增"agent 缓存"模块。给定目标版本 `v`（= `serverVersion()`）：
  - 缓存命中（`/var/lib/nft/agent-cache/<v>/nft-agent` + sha 记录）→ 直接用。
...
- `server` 角色：下载 **nft-server + nft-agent**（面板机 self-node daemon 需 nft-agent）。`nft-server.service` 跑 nft-server；`nft-daemon.service` 跑 `nft-agent daemon`。写 self-node 的 `{version, sha}`。
...
- `nft-upgrade`：按已装角色拉对应二进制并更新各自 `{version, sha}` 文件。
...
- 部署：`ssh hosthatch-jp nft-upgrade`（拉两者、更新 self-node 标识）。
...
- **持久化**：把所选 proxy 写入安装后的 `nft-upgrade` 脚本/配置（如 `/etc/nft/gh-proxy`），使后续自升级（update / update-script）继续用同一代理，无需每次重传。
