...
- `nft` (TUI) — 直接 `nft.Apply` 写本机 nftables；state 在 `/var/lib/nft/rules.json`
- `nft-agent` — HTTP :7878 接收 server 推送，直接 `nft.Apply` 写本机；state 在 `/var/lib/nft/agent-state.json`
- `nft-server` — Web UI :8080；通过 `local://` sentinel scheme **进程内嵌** agent 操作本机；state 在 `/var/lib/nft/embedded-agent-state.json`
...
**单二进制 `nft` + subcommand 切换角色 + 本机 host daemon 作为 nftables 的唯一控制器**：

- 一份发布产物
- 默认 `nft` 进 TUI（保留现有卖点）
- `nft daemon` 作为唯一操作 nftables 的进程（systemd 跑）
- `nft server` / `daemon --listen :7878` 作为 daemon 的"非 TUI 客户端"，分别处理 Web UI 和远程 panel 推送
...
```
nft                          → 默认进 TUI client
nft daemon                   → 守护进程（systemd 跑）。监听 unix socket，操作本机 nftables，
                                       是唯一调用 nft.Apply 的进程
nft daemon --listen ADDR --token-file PATH
...
nft apply                    → 兼容当前 systemd unit 调用入口；从 rules.json 读规则并通过
                                       daemon socket 提交后退出
nft --install-service        → 安装 nft-daemon.service（替代旧的 nft.service）
nft --uninstall-service      → 卸载
```

**等价替换**：
- 旧 `nft-agent --listen :7878 --token-file ...` ⇔ 新 `nft daemon --listen :7878 --token-file ...`
- 旧 `nft-server --addr :8080 ...` ⇔ 新 `nft server --addr :8080 ...`
- 旧 `nft --apply` ⇔ 新 `nft apply`
...
**Transport**：HTTP over Unix Socket，路径 `/var/run/nft.sock`，文件权限 `0660`、属主 `root:nft`（install.sh 创建 group，root 默认在 group 里；普通用户加入 group 后可跑 TUI 不带 sudo）。
...
唯一文件：`/var/lib/nft/state.json`，由 daemon 独占写入。schema：
...
| 旧文件 | 导入到 |
|---|---|
| `/var/lib/nft/rules.json` | `state.owners.tui` |
| `/var/lib/nft/agent-state.json` | `state.owners.panel` |
| `/var/lib/nft/embedded-agent-state.json` | `state.owners.panel` |
...
- TUI 启动时若 `GET /v1/health` 失败 → 报错并提示 `sudo systemctl start nft-daemon.service`，**不 fallback** 到直接管 nftables（保持单一控制路径）
...
- 本机节点 = 一个普通节点，address 为 `unix:///var/run/nft.sock`
- 远程节点 = HTTP URL，与现状一致
...
当 `nft daemon --listen :7878 --token-file ...` 时，daemon 在 unix socket 之外**额外**起一个 HTTP server：
...
| 现状 | 重构后 |
|---|---|
| `cmd/nft/` | **保留**，扩展为 dispatch：tui / daemon / server / apply / install-service |
| `cmd/nft-agent/` | **删除**。功能由 `nft daemon --listen ...` 取代 |
| `cmd/nft-server/` | **删除**。功能由 `nft server ...` 取代 |

`install.sh` 升级时检测到旧 `/usr/local/sbin/nft-agent` 或 `nft-server` 存在 → 提示用户它们已被 `nft` 子命令替代 → 自动清理。

### Daemon systemd 单元

`/etc/systemd/system/nft-daemon.service`（由 install.sh 写入）：
...
[Service]
ExecStart=/usr/local/sbin/nft daemon
Restart=on-failure
...
`nft-server.service`（仅 server 角色时存在）：
...
[Service]
ExecStart=/usr/local/sbin/nft server --addr :8080
Restart=on-failure
...
agent role 不再有独立 service —— 通过修改 `nft-daemon.service` 的 `ExecStart` 为 `nft daemon --listen :7878 --token-file ...` 即可。install.sh 提供 `install.sh agent --token ...` 操作来 patch daemon unit。

### install.sh 流程

| 子命令 | 行为 |
|---|---|
| `install.sh` (无 mode) / `install.sh tui` | 下载二进制 + 写 `nft-daemon.service` + `enable --now` daemon + 提示用户运行 `nft` |
| `install.sh server [--addr :8080] [--password ...]` | 在 daemon 已装的基础上写 `nft-server.service` + `enable --now` |
...
| `install.sh uninstall server` | `disable --now nft-server.service` + 删 unit。daemon 保留 |
...
- daemon 创建 `nft` system group（若不存在），socket 文件 group 即此 group
...
| 7 | 改 | `cmd/nft/` | dispatch：tui / daemon / server / apply / install-service |
...
- **场景 C：迁移旧 nft-server**
  ...
  4. server 启动时直接复用 panel.db，业务数据保留；本机节点 address 由 `local://` 改为 `unix:///var/run/nft.sock`（DB migration）
- **场景 D：daemon 重启后状态恢复**
  1. 各 owner 各加几条规则
  2. `systemctl restart nft-daemon` 
...
- 落地建议**分阶段单 PR / 单 spec 推进**，每阶段产物可独立 ship：
  - **A. daemon 骨架** — 新增 `internal/daemon` + socket listener + 简化的 owner-less ruleset apply。`nft daemon` 子命令可独立运行
