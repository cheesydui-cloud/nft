...
`nft` daemon 当前只维护自己的 `ip nft_forward` 表（`prerouting` nat dnat + `postrouting` nat masquerade）。在干净 Debian/Ubuntu 主机上 `filter` 表的 FORWARD chain 默认 policy=ACCEPT，DNAT 后的包能正常 forward。

但只要主机上装了**任何修改 FORWARD chain default policy 为 drop 的工具**——Docker / ufw / firewalld 这一类——nft 的 DNAT 流量就会在 FORWARD hook 处被静默丢弃，表现为"client SYN 重传，packet 进了 sg-cn2 但 DNAT 后没出去"。这在生产 Docker 主机上是常态：
...
用户的实际症状：装 nft + 配 DNAT → 转发不通。手动 `iptables -I DOCKER-USER ... -j ACCEPT` 后立刻通。
...
```
ip daddr <dest_ip> <proto-dport-match> counter accept comment "nft managed"
```
...
所有 shim 注入的 rule 共用 comment 字串 `nft managed`。cleanup 时按这个字串 grep handle：

```bash
nft -a list chain ip filter DOCKER-USER \
  | awk '/comment "nft managed"/ { ... extract handle ... }'
```
...
add rule ip filter DOCKER-USER ct state established,related counter accept comment "nft managed"
add rule ip filter DOCKER-USER ip daddr 10.20.1.20 tcp dport 8443 counter accept comment "nft managed"
add rule ip filter DOCKER-USER ip daddr 10.20.1.21 udp dport 53 counter accept comment "nft managed"
```
...
`install.sh uninstall daemon --purge` 通过 `systemctl disable --now nft-daemon.service` 触发 daemon Stop → CleanupAll 自动执行。
...
- `parseHandles`: 从 `nft -a list chain` 输出 fixture 中提取 comment 含 `nft managed` 的 rule 的 handle
...
docker compose exec daemon bash -c '
  ...
  nft list chain ip filter DOCKER-USER | grep -q "nft managed" \
    || { echo "shim 未注入"; exit 1; }
  # 提交空 ruleset，触发删除
  curl -sf --unix-socket /var/run/nft.sock \
       -X POST http://daemon/v1/ruleset/tui \
       -d "{\"rules\":[]}"
  # 验证 DOCKER-USER 里 nft managed 全部消失（ct state 兜底也走）
  ! nft list chain ip filter DOCKER-USER | grep -q "nft managed" \
...
| 风险 | 缓解 |
|---|---|
| Docker 重启时清空 DOCKER-USER 内容 | 实测 Docker 重启不动 DOCKER-USER 自定义内容；万一被清，下次 daemon Apply（rule 变化）自动 re-sync。用户也可手动 `systemctl restart nft-daemon.service` 强制 re-sync |
| daemon Stop 没走完 cleanup（崩溃 / kill -9） | 下次 Start 时 SyncAll 的"删旧加新"语义自动收口，残留不累积 |
| shim 与未来 daemon 版本不一致（升级时 comment 字串若改） | comment 字串 `nft managed` 锁定为 stable string，未来版本不改；如必须改，提供 migration（先按旧 comment 清，再按新 comment 加） |
