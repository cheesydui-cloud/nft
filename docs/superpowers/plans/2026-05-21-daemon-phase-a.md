...
**Goal:** 落地 `internal/daemon` 包 + `nft daemon` 子命令：新增一个独立守护进程，监听 unix socket，提供 HTTP-over-unix-socket API 接收 ruleset 提交，调用 `nft.Apply` 写本机 nftables，并把 ruleset 持久化到 `/var/lib/nft/state.json`。**Phase A 完全不动现有 TUI / server / agent 代码与行为**；daemon 是新增的并行路径，旧三套二进制继续按现状跑。
...
| `cmd/nft/main.go` | **改** | 加 `daemon` 子命令分发（保留所有现有 flag） |
...
Mode 0660 + best-effort group chown lets users in a nft
...
## Task 5: `cmd/nft daemon` subcommand
...
func runDaemon() int {
	if os.Geteuid() != 0 {
		fmt.Fprintln(os.Stderr, "nft daemon 必须以 root 身份运行")
		return 1
	}
...
func runInstallService() int {
	...
	fmt.Println("已安装 systemd 单元 nft.service；规则将在开机时自动恢复")
	return 0
}
...
func promptPersist() bool {
	...
	fmt.Println("启用后将把本程序复制到 /usr/local/sbin/nft，")
...
Run:
```bash
go build -o /tmp/nft-build ./cmd/nft
ls -l /tmp/nft-build && rm /tmp/nft-build
```
...
```bash
go build -o /tmp/nft-build ./cmd/nft
/tmp/nft-build --help 2>&1 | head -10
/tmp/nft-build daemon --help 2>&1 | head -10
rm /tmp/nft-build
```
Expected:
...
git commit -m "cmd: add 'nft daemon' subcommand entry
...
# 7. Ctrl-C 终止 daemon → /tmp/nft.sock 应被清理
ls /tmp/nft.sock
...
# --apply 仍能工作
sudo ./build/nft --apply 2>&1 | head -2
```
...
- `nft daemon` 子命令可独立运行，持有 `/var/run/nft.sock`
- daemon 重启可从 `/var/lib/nft/state.json` 恢复
