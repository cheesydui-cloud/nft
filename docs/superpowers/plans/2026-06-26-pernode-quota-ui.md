...
Run: `grep -n 'port.range\|port_range' /Users/xjetry/work/vibe/nft/install.sh`
...
const portRangePart = node.port_range && node.port_range !== '10001-20000'
  ...
  : ''
const installCmd = `curl -fsSL ${proxyPrefix}https://raw.githubusercontent.com/cheesydi-cloud/nft/main/install.sh | bash -s agent \\\n  --panel-url ${panel_url} \\\n  --token ${node.secret}${portRangePart}${proxyPrefix ? ` \\\n  --gh-proxy ${proxyPrefix}` : ''}`
```

- [ ] **Step 5: 运行后端测试**

Run: `cd /Users/xjetry/work/vibe/nft && go test ./internal/... -count=1`
Expected: PASS
