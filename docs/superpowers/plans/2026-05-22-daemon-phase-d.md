...
- `cmd/nft-server` disappears. Its bootstrap (panel DB, embedded apply, password reset) folds into `nft server`, but with no embedded agent — the local host is just another node with `address = unix:///var/run/nft.sock`.
...
- `internal/db/migrations.go` (or wherever migrations live — confirm at task time) — add migration that rewrites `nodes.address` from `local://...` to `unix:///var/run/nft.sock`.
- `cmd/nft/main.go` — add `server` subcommand dispatch; add `--listen` / `--token-file` to `daemon` subcommand; delete dead helpers (`preflight`, `promptPersist`, `promptYes`, `runApply`, `runInstallService`, `runUninstall`); fold `cmd/nft-server/main.go` bootstrap (admin reset, password seed, panel start) into a new `runServer` and `runResetAdmin`.
...
Add the missing imports at the top of the file if not already present: `bytes`, `io`, `net`, `net/http`, `net/http/httptest`, `path/filepath`, `nft/internal/daemonclient`, `nft/internal/nft`.
...
const DefaultSocketPath = "/var/run/nft.sock"
...
```sql
UPDATE nodes SET address = 'unix:///var/run/nft.sock'
WHERE address = 'local://';
```
...
if d.httpListen != "" {
    go func() {
        ...
        log.Printf("nft daemon listening on %s (http)", d.httpListen)
...
fs.StringVar(&tokenFile,  "token-file", "/etc/nft/daemon.token", "bearer token file (required when --listen is set)")
fs.Parse(args)
...
Copy `bootstrap` and `runResetAdmin` verbatim from `cmd/nft-server/main.go`. They become package-level functions in `cmd/nft/main.go`.

`runApplyCompat`: this preserves the `nft apply` entrypoint used by older systemd units. It reads rules from a path (default `/etc/nft/rules.json`), opens a `daemonclient` to `DefaultSocketPath`, and POSTs them as owner `tui`. If you don't have time to write this in this sub-step, leave the original `runApply` in place but rewire it to use daemonclient (don't touch the legacy preflight helpers yet — they go in 9c).
...
1. Start daemon: `sudo nft daemon`.
2. Start server: `sudo nft server --addr :8080`.
3. Open `http://host:8080`, log in (admin password printed on first boot).
4. Confirm that `localhost` node is auto-registered with address `unix:///var/run/nft.sock`.
5. Create a forward via the panel UI.
6. `curl --unix-socket /var/run/nft.sock http://daemon/v1/ruleset` must show the rule under owner `panel`.
7. In a second terminal start `nft` (TUI). Add one more rule.
8. `curl ... /v1/ruleset` must show both owners with their respective rules.

Add *"Remote daemon (HTTP) role"*:

1. On the panel host: `nft server --addr :8080`.
2. On a second host: `sudo nft daemon --listen :7878 --token-file /etc/nft/daemon.token` (with a token of your choice written to that file).
...
1. `grep -RIn "nft/internal/agent" .` returns nothing.
...
6. `go vet ./...` is clean.
7. Manual smoke: `sudo nft daemon &` then `sudo nft server --addr :8080`; verify the panel comes up, the local node uses `unix://` address, and adding a forward applies through nftables on the host.
...
- Retire the legacy `nft.service` unit.
