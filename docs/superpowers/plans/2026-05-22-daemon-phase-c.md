...
| 3 | **TUI 启动不再 prompt 安装 `nft.service`** | 持久化职责由 daemon 接管（daemon 有自己的 systemd unit），TUI 不再需要装老的 nft.service |
| 4 | **TUI 不再做 root + nft preflight** | daemon 自己做了；TUI 只看 socket 能不能连。连不上→报错，不 fallback |
| 5 | **`cmd/nft --apply` 暂不动** | 它仍走 `store.Load` + `nft.Apply` 直接路径，是旧 `nft.service` 的 ExecStart。Phase E (install.sh) 会替换 service 表面 |
| 6 | **`internal/store` 包不删** | `cmd/nft --apply` 和未迁移的 internal/agent 仍引用。删除是 Phase E 的事 |
...
| `cmd/nft/main.go` | **改** | `runTUI` 替换为：构造 daemonclient → Health 检查 → 失败时报错 + 提示启动 daemon → 不 prompt 安装 nft.service → 不调 store.Load/nft.Apply → `tui.Run(client)` |
...
// Client speaks HTTP over a unix socket to the local nft daemon.
...
最终 tui.go 顶部 imports（需要新增 `"nft/internal/daemonclient"`）：
...
> 把 tui_test.go 顶部 import 块加上 `"nft/internal/daemonclient"`（如果尚未含）。
...
## Task 3: cmd/nft `runTUI` 改造

`runTUI` 不再 store.Load / nft.Apply / prompt 安装 nft.service。改为：构造 daemonclient → Health 检查 → 失败时报错 + 提示用户启动 daemon service → 成功时 `tui.Run(client)`。

**Files:**
- Modify: `cmd/nft/main.go`

- [ ] **Step 1: 改 `cmd/nft/main.go` 顶部 import**
...
func runTUI() int {
	...
	if !systemd.Installed() {
		if promptPersist() {
			...
			fmt.Println("已启用开机持久化：systemd 单元 nft.service")
		}
	}
...
func runTUI() int {
	...
	if err := client.Health(); err != nil {
		...
		fmt.Fprintln(os.Stderr, "或者临时：sudo nft daemon")
		return 1
	}
...
longer offers to install the legacy nft.service, and no
...
1. 改 1 个文件（cmd/nft/main.go）。**其他文件字节级不变**。
...
- **仅 unix socket** — 远程接入（HTTP + Bearer token）会在接入 server/agent 时再加
- **无认证** — 只有 socket 文件权限是访问控制（生产部署只让 root + nft group 用户能连）
- **TUI 已接入 daemon** — 运行 `sudo nft`（默认子命令）会通过 unix socket 与 daemon 对话；daemon 没起会立即报错，不再 fallback 直接管 nftables
...
sudo nft speaks to the daemon over the unix socket, and
...
- 切回 curl 验证：`curl -s --unix-socket /tmp/nft.sock http://unix/v1/ruleset` 反映改动
...
应该立即报错并提示：`请先启动 daemon：sudo systemctl start nft-daemon.service`
...
- TUI 是 daemon 的第一个真实 client；删除了 TUI 内部的 store / nft.Apply / nft.service 安装逻辑
...
- 删除 `cmd/nft-agent` 和 `cmd/nft-server` 两个 binary 目录，归并到 `cmd/nft` 的 server / 远程 daemon subcommand 下
...
- [x] `cmd/nft --apply` 不动（仍走 store.Load + nft.Apply）— Phase E install.sh 重写时同步处理
