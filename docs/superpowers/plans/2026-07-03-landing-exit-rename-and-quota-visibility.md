...
- 所有 Go 测试命令在仓库根目录 `/Users/xjetry/work/vibe/nft` 运行。
...
在 `internal/server/landing_sync_test.go` 末尾追加（文件已 import `httptest`、`json`、`db`；若 `nft/internal/landing` 未 import 则补上）：
...
（`nft/internal/landing` 已在该文件 import——`landing.Node` 在 stale 回退路径使用。）
...
Run: `cd /Users/xjetry/work/vibe/nft/web && npm run build`
...
Run: `cd /Users/xjetry/work/vibe/nft && go test ./...`
Expected: 全部 PASS
