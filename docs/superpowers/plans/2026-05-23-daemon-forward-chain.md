...
// nft keep working on hosts where some other tool has set the
...
// this exact comment.
const OwnerComment = "nft managed"
...
func TestParseShimHandlesIgnoresUnrelatedRules(t *testing.T) {
	...
	handles := parseShimHandles(out)
	if len(handles) != 0 {
		t.Fatalf("expected 0 handles (no nft managed), got %v", handles)
	}
}
...
func TestParseShimHandlesPicksOwnerTagged(t *testing.T) {
	...
	chain DOCKER-USER {
		...
		ct state established,related counter accept comment "nft managed" # handle 12
		ip daddr 10.0.0.1 tcp dport 80 counter accept comment "nft managed" # handle 17
...
func TestRenderShimScriptInsertsTCPRule(t *testing.T) {
	...
	want := `add rule ip filter DOCKER-USER ip daddr 10.20.1.20 tcp dport 8443 counter accept comment "nft managed"`
...
func TestRenderShimScriptInsertsTCPUDPRule(t *testing.T) {
	...
	script := renderShimScript("ip", "filter", "DOCKER-USER", rules, nil)
	want := `add rule ip filter DOCKER-USER ip daddr 10.20.1.20 meta l4proto { tcp, udp } th dport 8443 counter accept comment "nft managed"`
	if !strings.Contains(script, want) {
		t.Fatalf("expected line %q in:\n%s", want, script)
	}
}
...
func TestDockerUserShimSyncDeletesStaleThenAdds(t *testing.T) {
	r := &recorder{
		listOut: `table ip filter {
	chain DOCKER-USER {
		ct state established,related counter accept comment "nft managed" # handle 7
		ip daddr 10.0.0.1 tcp dport 80 counter accept comment "nft managed" # handle 8
	}
}`,
	}
	s := newDockerUserShimWith(r)
...
func TestUfwShimCleanupRemovesAll(t *testing.T) {
	r := &recorder{
		listOut: `table ip filter {
	chain ufw-user-forward {
		ct state established,related counter accept comment "nft managed" # handle 22
	}
}`,
	}
	s := newUfwShimWith(r)
...
docker compose exec daemon bash -c '
  ...
  # 验证 DOCKER-USER 里出现 nft managed rule
  if ! nft list chain ip filter DOCKER-USER | grep -q "nft managed"; then
...
daemon 启动 / 每次 apply 时会自动同步 `DOCKER-USER`（如果装了 Docker）和 `ufw-user-forward`（如果装了 ufw）chain 里的放行规则，让 nft 的 DNAT 流量穿透 Docker / ufw 把 FORWARD policy 设成 drop 的环境。daemon 卸载或停止时这些规则自动清除。

无 Docker / 无 ufw 的纯净系统：daemon 不动任何 chain，启动日志仅打印 `shim docker-user: target chain not found, skipping`。

如果你装了别的 firewall 工具（firewalld 等）让 FORWARD policy=drop 但 daemon 不能自动处理，启动日志会有一行 `WARN: FORWARD chain has drop policy but no known firewall shim detected`——这时需要手动在你的 firewall 里放行 nft DNAT 后的流量。
```
...
3. **Type / 名称一致性**:
   ...
   - `OwnerComment = "nft managed"` 在 Task 1 定义、Task 2 parseShimHandles 使用、Task 9 docker/test.sh 验证 ✓
