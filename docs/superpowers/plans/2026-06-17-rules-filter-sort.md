...
```bash
grep -n "func (s \*Server) apiListRules\|^func " /Users/xjetry/work/vibe/nft/internal/server/api.go | head -20
```
...
```bash
grep "strconv" /Users/xjetry/work/vibe/nft/internal/server/api.go | head -3
```
...
```bash
cd /Users/xjetry/work/vibe/nft/web && npm run dev
```
