...
（该文件需要 import `nft/internal/nft`；`createStandaloneRuleHop` 的返回值/签名以 `shared_test.go` 中的定义为准，若第二返回值不是 error 需相应调整。）
...
（`tc_test.go` 需要 import `reflect` 与 `nft/internal/nft`。）
