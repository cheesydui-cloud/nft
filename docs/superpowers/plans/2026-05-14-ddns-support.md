...
**Goal:** 让 nft 在 TUI、agent、server 三种形态下都能把 hostname/DDNS 域名当作转发目标，并在底层 IP 变化时自动重建 nftables 规则，无需人工介入。
...
// We return only IPv4 because nft's nftables rules live in the `ip` family.
...
加 import `"nft/internal/resolver"`。
...
import 处加 `"context"` 和 `"nft/internal/resolver"`。
...
补 `import "nft/internal/resolver"` 与 `"strings"`（若未导入）。
...
```bash
GOOS=linux GOARCH=amd64 go build -trimpath -ldflags="-s -w" -o build/nft ./cmd/nft
