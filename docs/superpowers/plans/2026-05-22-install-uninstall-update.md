...
rollback_update() {
  ...
  if [[ -f "$INSTALL_DIR/nft.bak" ]]; then
    mv -f "$INSTALL_DIR/nft.bak" "$INSTALL_DIR/nft"
  fi
  systemctl restart nft-daemon.service 2>/dev/null || true
  if systemctl list-unit-files --no-legend \
     | grep -q '^nft-server\.service '; then
...
do_update() {
  # ---- 前置探测 ----
  [[ -x "$INSTALL_DIR/nft" ]] \
    || die "未安装：$INSTALL_DIR/nft 不存在；请先 install.sh tui/server/agent"
  systemctl list-unit-files --no-legend | grep -q '^nft-daemon\.service ' \
  ...
  file "$tmp/nft" | grep -q 'ELF 64-bit LSB.*x86-64' \
    || die "下载到的二进制不是 ELF 64-bit x86-64（content: $(file "$tmp/nft"))"

  # ---- exec 自检：只要不是 "exec format error" 就通过 ----
  # nft 没 --version flag；Go flag 包遇到未知 flag 退 2 但能 exec，足够证明二进制可运行。
  if ! "$tmp/nft" --version >/dev/null 2>&1; then
    if [[ $? -gt 125 ]]; then
  ...
  # ---- 备份旧二进制 ----
  note "[3/5] 备份旧二进制到 $INSTALL_DIR/nft.bak ..."
  cp -a "$INSTALL_DIR/nft" "$INSTALL_DIR/nft.bak"
  ...
  systemctl is-active --quiet nft-daemon.service \
    && curl -sf --unix-socket /var/run/nft.sock \
            http://daemon/v1/health 2>/dev/null \
       | grep -q '"ok":true' \
    || rollback_update
  ...
  # ---- 成功收尾 ----
  trap 'rm -rf "$tmp"' EXIT
  rm -f "$INSTALL_DIR/nft.bak"
  local sha size
  sha=$(sha256sum "$INSTALL_DIR/nft" | awk '{print $1}')
  size=$(stat -c %s "$INSTALL_DIR/nft" 2>/dev/null || stat -f %z "$INSTALL_DIR/nft")
  ok "===== Update 完成 ====="
  echo "二进制 sha256: $sha"
...
`docker/Dockerfile` 已经把 `nft` 放到 `/usr/local/sbin/nft`，但**没有装 install.sh**——docker fixture 跑的是 compose `command:` 而不是 install.sh。
...
docker compose exec daemon bash -c '
  ...
  mkdir -p /tmp/relmock
  cp /usr/local/sbin/nft /tmp/relmock/nft
  ( cd /tmp/relmock && sha256sum nft > SHA256SUMS )
...
docker compose exec daemon bash -c '
  ...
  /usr/local/sbin/nft-install --help | grep -q "update" \
    || { echo "usage 缺 update"; exit 1; }
  # 错误用法：--release 与 update 互斥
  if /usr/local/sbin/nft-install update --release v1.0 2>&1 \
  ...
  if /usr/local/sbin/nft-install update --purge 2>&1 \
  ...
  sleep 1
  # 预探：URL 替换工作
  curl -sf http://127.0.0.1:8765/nft -o /tmp/check.bin
  test "$(sha256sum /tmp/check.bin | awk "{print \$1}")" \
       = "$(sha256sum /usr/local/sbin/nft | awk "{print \$1}")" \
...
```bash
git add install.sh
git commit -m "install: add --purge to uninstall agent (clear /etc/nft and daemon panel segment)"
```
...
detect_existing_roles() {
  ...
  if [[ -f "$SYSTEMD_DIR/nft-daemon.service" ]] \
     && grep -q -- '--listen' "$SYSTEMD_DIR/nft-daemon.service"; then
    roles+=(agent)
  fi
  echo "${roles[@]}"
}
...
```bash
grep -n '^## 升级与迁移' /Users/xjetry/work/vibe/nft/README.md
```
...
行为：拉 GitHub latest 二进制 → sha256 校验 → ELF x86-64 架构校验 → 备份旧二进制到 `/usr/local/sbin/nft.bak` → 原子替换 → `systemctl restart nft-daemon.service`（+ `nft-server.service` 如有）→ 10 秒 health-check。失败自动回滚到旧二进制并重启。
...
sudo bash install.sh uninstall agent  --purge   # 卸 agent 同时清 /etc/nft + daemon panel 段
...
sudo nft          # 进 TUI，按 'a' 加一条转发规则（如 :10000 → 1.2.3.4:80）
...
\`\`\`bash
sudo bash install.sh update
sudo nft          # 回 TUI 看刚才那条规则是否还在
\`\`\`

预期：规则仍存在，新版二进制 sha 已经变化。检查 `journalctl -u nft-daemon.service --since '5 minutes ago'` 看到正常启动日志。

### 2. agent 角色 update 保留 token

\`\`\`bash
sudo bash install.sh agent --port 7878 --token deadbeef...64hex
ls -l /etc/nft/daemon.token   # 记录 inode 和 mtime
sudo bash install.sh update
ls -l /etc/nft/daemon.token   # 应当与上面一致，inode 不变
cat /etc/systemd/system/nft-daemon.service | grep ExecStart
...
systemctl list-unit-files | grep nft
# 应仅余 nft-daemon.service
sudo nft          # 应同时看到规则 A 和规则 B（state.json 完整保留）
\`\`\`
...
sudo sed -i 's|/nft daemon|/nft daemon --bogus-flag|' \
    /etc/systemd/system/nft-daemon.service
...
- `/usr/local/sbin/nft` 的 sha 与 update 前一致
- daemon 仍然起不来（unit 被改坏了）—— **这是单元损坏不是 update 单元错**；人工恢复 unit：

\`\`\`bash
sudo sed -i 's| --bogus-flag||' /etc/systemd/system/nft-daemon.service
sudo systemctl daemon-reload
sudo systemctl restart nft-daemon.service
\`\`\`
