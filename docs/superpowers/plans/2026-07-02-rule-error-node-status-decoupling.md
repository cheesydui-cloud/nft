...
- 模块路径 `nft`；测试用 `go test ./internal/...`，构建用 `go build ./...`。
...
确认 `internal/server/shared.go` import 块含 `"nft/internal/resolver"`（若无则加）。在 `parseExit` 中 `if host == ""` 判空之后、`return host, port, nil` 之前插入：
...
确认 `internal/daemon/handlers.go` import 块含 `"nft/internal/resolver"`（若缺则加）。
