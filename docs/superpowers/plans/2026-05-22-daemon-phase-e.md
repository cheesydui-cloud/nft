...
**Goal:** Retire the pre-daemon-ization surface of the project. Remove the now-orphan `apply` subcommand, `internal/store`, and `internal/systemd` packages. Rewrite `install.sh` so a single `nft` binary supports tui / server / agent on the same host with daemon-first installation and clean migration from the old multi-binary layout. Rewrite the docker fixture and `README.md` to match.

**Architecture:**
- The daemon is the only nftables controller and persists its own state — there is no longer a need for a "boot-time apply" path that reads `/etc/nft/rules.json` directly. `nft apply` and the `internal/store` package are deleted.
- `internal/systemd` had no callers after Phase D removed the install-service / uninstall-service handlers; it is deleted.
- `install.sh` becomes a single-binary installer with three role sub-commands (`tui`, `server`, `agent`) that stack: each role assumes the daemon is already installed. The script writes `nft-daemon.service` (and, for the server role, `nft-server.service`); the agent role patches the daemon unit's `ExecStart` to add `--listen` and a token file. The script detects and removes the legacy `nft.service` / `nft-server.service` / `nft-agent.service` units and the legacy `/usr/local/sbin/{nft-agent,nft-server}` binaries.
- `docker/` becomes a single `Dockerfile` building the `nft` binary, with `docker-compose.yml` standing up a daemon container + a server container; `test.sh` is adapted to drive the new architecture.
...
**Created:**
- `docker/Dockerfile` (single image for `nft`).

**Modified:**
- `cmd/nft/main.go` — remove `apply` from the dispatcher and remove `runApplyCompat`; drop the `internal/store` and `internal/daemonclient` imports if they become dead in this file (daemonclient is still used by `runTUI`, so it stays).
...
**Files:**
- Modify: `cmd/nft/main.go`
- Delete: `internal/store/` (whole directory)

**Context:** The `apply` subcommand was the entry point of the legacy `nft.service` systemd unit (`ExecStart=nft --apply`, later `nft apply`). It read `/etc/nft/rules.json` through `internal/store.Load()` and posted the rules to the daemon. With the daemon now persisting its own state and starting from systemd, there is no second source-of-truth — the apply path is doing the same work the daemon already does at boot. Deleting it removes the last consumer of `internal/store`, so the package goes with it.

- [ ] **Step 1: Remove the dispatch and the function**

Open `cmd/nft/main.go`. In `main()`, delete the `case "apply":` arm. Delete the `runApplyCompat` function entirely. After this edit, search for orphan imports — `internal/store` is the most likely casualty. `internal/daemonclient` is still used by `runTUI`, so leave it.
...
```bash
git add cmd/nft/main.go internal/store
...
```
grep -RIn 'nft/internal/systemd' --include='*.go' .
```
...
[Service]
ExecStart=$INSTALL_DIR/nft daemon$extra_args
Restart=on-failure
...
remove_legacy_units() {
  ...
  # has since absorbed. Remove them so PATH doesn't shadow nft.
...
download nft binary; install to /usr/local/sbin
mode dispatch:
  tui    -> ensure daemon unit, enable, print "run sudo nft"
    ...
    daemon)
      if systemctl is-active --quiet nft-server.service \
         || systemctl list-unit-files --no-legend \
            | grep -qE '^nft-server\.service '; then
...
note "[1/3] 下载 nft ($RELEASE) ..."
curl -fL --progress-bar "$base/nft" -o "$tmp/nft" \
  || die "下载失败: $base/nft"

note "[2/3] 校验 sha256 ..."
if curl -fLs "$base/SHA256SUMS" -o "$tmp/SHA256SUMS" 2>/dev/null; then
  (cd "$tmp" && grep -E '  nft$' SHA256SUMS | sha256sum -c -) \
    || die "sha256 校验失败"
else
  echo "    (SHA256SUMS 不可用，跳过校验)"
fi

note "[3/3] 安装到 $INSTALL_DIR/nft ..."
install -m 0755 "$tmp/nft" "$INSTALL_DIR/nft"
...
daemon unit: nft-daemon.service
server unit: nft-server.service
首次启动的 admin 密码: journalctl -u nft-server.service | grep 密 \\
  （或查看 server 启动日志）
EOF
    ;;
  ...
  agent)
    mkdir -p /etc/nft
    install -m 0600 /dev/stdin /etc/nft/daemon.token <<<"$token"
    write_daemon_unit " --listen :$port --token-file /etc/nft/daemon.token"
    systemctl daemon-reload
    systemctl enable --now nft-daemon.service
    cat <<EOF
...
**Context:** The docker fixture today builds two separate images (agent, server) from the deprecated binaries. After this task it builds one image — the same `nft` binary — and runs two containers from it: one as a daemon, one as a server, with the daemon's unix socket bind-mounted between them.
...
It almost certainly drives the old `nft-server` HTTP API. Note what scenarios it asserts (login, add node, add forward, etc.) — you'll keep those scenarios, just talk to `nft server` instead of `nft-server`.
...
services:
  daemon:
    ...
    network_mode: host
    volumes:
      - daemon-state:/var/lib/nft
      - daemon-run:/var/run
    command: ["/usr/local/sbin/nft", "daemon"]
  ...
  server:
    ...
    volumes:
      ...
      - server-data:/var/lib/nft
    command: ["/usr/local/sbin/nft", "server", "--addr", ":8080"]
...
(Both services use `network_mode: host` because the daemon needs to manage host nftables and the panel binds the operator-visible HTTP port. Shared `daemon-run` volume gives the server access to `/var/run/nft.sock`.)
...
- Replace `docker compose exec server /usr/local/sbin/nft-server ...` with `docker compose exec server /usr/local/sbin/nft server ...` (or just exercise the running container's HTTP API).
- Replace `docker compose exec agent ...` with `docker compose exec daemon /usr/local/sbin/nft daemon --help` (smoke check) plus curl against the unix socket through `docker compose exec daemon curl --unix-socket /var/run/nft.sock http://daemon/v1/health`.
...
3. `docker compose exec daemon curl --unix-socket /var/run/nft.sock http://daemon/v1/health` returns `{"ok":true}`.
...
Note which sections describe binary-specific surface that needs to go (anything mentioning `nft-server`, `nft-agent` as binaries, or `nft.service` as a unit).
...
## 架构（一图 + 200 字）
- nft daemon：唯一调用 nftables / tc 的进程，监听 /var/run/nft.sock；可选 --listen :7878 接受 HTTP 远程推送。
- TUI / server 都是 daemon 的 client；server 通过 daemon 推送规则到所有节点。
- owner-segmented ruleset：tui / panel 共存，按端口冲突，daemon 合并后下发。

## 命令表面
- nft                  默认进 TUI（要求 daemon 已运行）
- nft daemon           前台启动 daemon（systemd 通常负责）
- nft daemon --listen :7878 --token-file <path>  agent 角色
- nft server [--addr :8080] [--db <path>]        web 面板

## 配置 / 持久化
- daemon state: /var/lib/nft/state.json
- daemon socket: /var/run/nft.sock (group nft, mode 0660)
- agent token: /etc/nft/daemon.token (mode 0600)
- panel DB: /var/lib/nft/panel.db

## 升级 / 迁移
- install.sh 自动 disable + 删除旧的 nft.service / nft-server.service / nft-agent.service
- /usr/local/sbin/{nft-agent,nft-server} 旧二进制被 install.sh 自动清理
- daemon 首次启动会把旧 /etc/nft/rules.json / /var/lib/nft/agent-state.json 自动迁入新 state.json 并把原文件重命名为 .migrated
...
Find the existing "Known limits" or equivalent section. Remove the bullet that pointed at install.sh + nft.service refactor as "next phase" — that bullet is now obsolete. Replace it with something like:
...
1. `grep -RIn "nft/internal/store\|nft/internal/systemd" --include='*.go' .` returns nothing.
