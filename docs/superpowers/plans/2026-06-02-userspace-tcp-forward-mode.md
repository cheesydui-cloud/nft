...
`Daemon` 结构体里 `applier Applier` 改为 `dp Dataplane`;`countersFn` 字段类型改为 `func() ([]forward.Counter, error)`(import `nft/internal/forward`)。
...
(import `nft/internal/forward`;移除不再使用的 `defaultCounters` 引用。)
...
(确保 `state_test.go` 已 import `nft/internal/nft`、`os`、`path/filepath`。)
...
```
(import `strconv`、`log`、`nft/internal/wsproto` 视需要补。)
