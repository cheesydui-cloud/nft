...
$COMPOSE exec daemon \
  curl --unix-socket /var/run/nft.sock \
...
$COMPOSE exec daemon bash -c '
  ...
  for _ in $(seq 1 20); do
    curl -sf http://127.0.0.1:8765/nft -o /tmp/check.bin && break
    sleep 0.2
  done
  ...
  grep -q "NFTF_RELEASE_BASE_URL" /usr/local/sbin/nft-install \
...
$COMPOSE exec -T server sqlite3 /var/lib/nft/panel.db \
...
$COMPOSE exec daemon curl -sf --unix-socket /var/run/nft.sock \
  -X POST -H "Content-Type: application/json" \
  http://daemon/v1/ruleset/tui \
...
$COMPOSE exec -T daemon sh -c "
  set -e
  mkdir -p /etc/nft
  printf '%s' '${NODE_TOKEN}' > /etc/nft/panel.token
  pkill -TERM -f 'nft daemon' || true
  ...
  for _ in \$(seq 1 20); do
    [ ! -S /var/run/nft.sock ] && break
    sleep 0.2
  done
  nohup /usr/local/sbin/nft daemon \
...
ONLINE="$($COMPOSE exec -T server sqlite3 /var/lib/nft/panel.db \
...
IMPORTED="$($COMPOSE exec -T server sqlite3 /var/lib/nft/panel.db \
...
$COMPOSE exec -T daemon sh -c '
  set -e
  grep -q "\"tui\"" /var/lib/nft/state.json \
    || { echo "demote 后 tui 段缺失"; cat /var/lib/nft/state.json; exit 1; }
  grep -q "10.20.1.21" /var/lib/nft/state.json \
    || { echo "demote 未保留 panel 规则"; cat /var/lib/nft/state.json; exit 1; }
