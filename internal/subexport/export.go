...
func buildClashYAML(nodes []Node, opt Options, global bool) string {
	...
	// Header comments — aligned with mieru-panel template.
	fmt.Fprintf(&b, "# Mihomo / Clash Meta — generated by %s\n", brand)
	fmt.Fprintf(&b, "# user: %s\n", user)
	if global {
		b.WriteString("# purpose: full tunnel · no China DIRECT · DNS via proxy path\n")
	} else {
	...
	// DNS block — mieru-panel style fake-ip + CN fallback (split) or overseas DoH (global).
	b.WriteString("dns:\n")
...
func yamlQuoteIfNeeded(s string) string {
	// Always quote proxy names (mieru-panel style; safe for CJK / spaces).
