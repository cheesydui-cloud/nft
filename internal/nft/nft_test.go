...
func TestResolveHostsFallsBackToAAAA(t *testing.T) {
	...
	rules := []Rule{{Proto: "tcp", SrcPort: 80, DestHost: "v6only.example", DestPort: 80}}
	out, changed, err := ResolveHosts(context.Background(), rules, r)
	if err != nil {
		t.Fatal(err)
	}
	if !changed || out[0].DestIP != "2001:db8::9" {
		t.Fatalf("want AAAA fallback, got changed=%v ip=%q", changed, out[0].DestIP)
	}
}
